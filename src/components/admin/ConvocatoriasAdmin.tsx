...
import { ConvocatoriaModal } from '../common/ConvocatoriaModal';
...
import { AdminTableSkeleton, StatsSkeleton } from '../common/Skeletons';
import { ConfirmationModal } from '../common/ConfirmationModal';

const API_URL = `https://${projectId}.supabase.co/functions/v1/make-server-f99e977c`;

interface ConvocatoriasAdminProps {
  currentUser?: any;
}

export function ConvocatoriasAdmin({ currentUser }: ConvocatoriasAdminProps) {
  ...
  const handleToggleVisibility = (convocatoria: any) => {
    ...
    setConfirmation({
      isOpen: true,
      title: isVisible ? 'Ocultar Convocatoria' : 'Publicar Convocatoria',
      description: confirmMessage,
      confirmText: isVisible ? 'Ocultar' : 'Publicar',
      variant: 'default',
      onConfirm: async () => {
  ...
  const handleDeleteConvocatoria = (id: string, title: string) => {
    setConfirmation({
      isOpen: true,
      title: 'Eliminar Convocatoria',
      description: '¿Está seguro de eliminar esta convocatoria permanentemente? Esta acción no se puede deshacer.',
