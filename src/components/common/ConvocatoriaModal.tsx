...
export function ConvocatoriaModal({ convocatoria, onClose, onSave }: ConvocatoriaModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 2. Project-Area Consistency
    if (formData.projectId && formData.area) {
      const project = projects.find(p => p.id === formData.projectId);
      if (project) {
        const projectArea = areas.find(a => a.id === project.areaId);
        if (projectArea && projectArea.name !== formData.area) {
          alert(`Error: El proyecto seleccionado pertenece al área "${projectArea.name}", pero has seleccionado "${formData.area}". Coincidencia requerida.`);
          return;
        }
      }
    }
